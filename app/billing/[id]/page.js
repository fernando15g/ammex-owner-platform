import { getProjectBilling } from "@/lib/data";
...
export default async function ProjectBillingPage({ params }) {
  ...
  try { data = await getProjectBilling(params.id); } catch (e) { error = String(e.message || e); }
  return (
    <AppShell current="billing" subtitle="Due billing" title={data?.name || "Project billing"}>
