...
import { invoiceTemplateBuffer } from "@/lib/documents/invoiceTemplate";

const BLOB_KEY = "templates/proposal-template.xlsx";
const INVOICE_BLOB_KEY = "templates/invoice-template.xlsx";
...
// =============================================================================
// INVOICE TEMPLATE — same storage pattern as the proposal template, its own
// blob key and its own built-in fallback. The proposal path above is untouched;
// these are parallel so the two templates never step on each other.
// =============================================================================

// The template to build invoices from: the uploaded one if there is one, the
// version baked into the build otherwise.
export async function getInvoiceTemplateBuffer() {
  if (!blobConfigured()) {
    return { buffer: invoiceTemplateBuffer(), source: "built-in" };
  }
  try {
    const { list } = await import("@vercel/blob");
    const { blobs } = await list({ prefix: INVOICE_BLOB_KEY, limit: 1 });
    if (!blobs.length) {
      return { buffer: invoiceTemplateBuffer(), source: "built-in" };
    }
    const res = await fetch(blobs[0].url, { cache: "no-store" });
    ...
    // Never let a storage hiccup stop an invoice going out.
    console.error("[template] couldn't read the uploaded invoice template, using the built-in one:", e.message || e);
    return { buffer: invoiceTemplateBuffer(), source: "built-in", error: String(e.message || e) };
  }
}

// Save an uploaded invoice template. Overwrites the previous one.
export async function putInvoiceTemplate(buffer) {
  if (!blobConfigured()) {
    throw new Error(
      "Template storage isn't set up yet. In Vercel: your project → Storage → Create a Blob store. " +
      "Vercel adds the token for you; then redeploy."
  ...
  const { put } = await import("@vercel/blob");
  const blob = await put(INVOICE_BLOB_KEY, buffer, {
    access: "public",
    contentType: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    addRandomSuffix: false,
    allowOverwrite: true,
...
// Go back to the invoice version baked into the build.
export async function resetInvoiceTemplate() {
  if (!blobConfigured()) return { reset: true, source: "built-in" };
  try {
    const { del, list } = await import("@vercel/blob");
    const { blobs } = await list({ prefix: INVOICE_BLOB_KEY, limit: 1 });
    ...
    throw new Error(`Couldn't remove the uploaded invoice template: ${e.message || e}`);
  }
}
