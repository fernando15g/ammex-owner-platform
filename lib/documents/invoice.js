...
export async function buildInvoice({ project, bill, bills = [], lines = [] }) {
  ...
  ws.pageSetup = {
    ...
    // NO printArea: ExcelJS writes a malformed Print_Area defined name (it emits
    // $A1:$J31, or $$A$1:$$J$31 if you pass absolute refs), and Excel flags the
    // file as corrupt on open. The sheet only holds content in A1:J anyway, so
    // there is nothing to gain by declaring one.
  };
  ws.pageSetup.printTitlesRow = "1:11";
  // real page numbers, top-right where the template's static one sat
  // oddHeader alone covers every page unless differentOddEven is set, so one
  // entry keeps the XML minimal and avoids a mismatched even/odd pair.
  ws.headerFooter = { ...(ws.headerFooter || {}), oddHeader: "&R&\"Arial,Regular\"&10PAGE &P OF &N" };
...
// -----------------------------------------------------------------------------
// EXCELJS SCHEMA-ORDER FIX
//
// Setting fitToPage makes ExcelJS write <sheetPr><pageSetUpPr/><outlinePr/>,
// but the OOXML schema requires pageSetUpPr AFTER outlinePr. Excel enforces
// element order strictly and shows "We found a problem with some content" —
// recovery yields a blank workbook. openpyxl/lxml tolerate the misorder, which
// is why sandbox validation kept passing files Excel rejected. Proven by a
// feature-ladder test: template round-trip opened fine; adding fitToPage broke.
//
// The fix re-zips the workbook with pageSetUpPr moved to the END of sheetPr,
// which satisfies the schema sequence (tabColor?, outlinePr?, pageSetUpPr?).
// -----------------------------------------------------------------------------
import JSZip from "jszip";

export async function invoiceBuffer(wb) {
  const raw = await wb.xlsx.writeBuffer();
  const zip = await JSZip.loadAsync(raw);
  const fixes = [];
  zip.forEach((path) => {
    if (/^xl\/worksheets\/sheet\d+\.xml$/.test(path)) fixes.push(path);
  });
  for (const path of fixes) {
    let xml = await zip.file(path).async("string");
    // move pageSetUpPr to the end of sheetPr, whatever else sheetPr holds
    xml = xml.replace(/<sheetPr>(.*?)<\/sheetPr>/s, (m, inner) => {
      const pieces = inner.match(/<pageSetUpPr[^>]*\/>/);
      if (!pieces) return m;
      const rest = inner.replace(pieces[0], "");
      return `<sheetPr>${rest}${pieces[0]}</sheetPr>`;
    });
    zip.file(path, xml);
  }
