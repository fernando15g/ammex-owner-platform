...
// formulas, the same 10% retention rows computing the Total Due. She reviews and
...
// Totals row: F/H/J = SUM of the column; retention = SUM(*29*10%); Total Due =
// SUM(J29-J30). We fill ONLY A,B,C,D,E,G — the template computes everything else.
// =============================================================================
...
export async function buildInvoice({ project, bill, bills = [], lines = [] }) {
  ...
  const projId = project.projectId ? ` / ${project.projectId}` : "";
  ...
  // Rows shift down by `extra` if we inserted lines. Rebuild the SUM ranges and
  // keep the template's 10% retention and Total Due formulas.
  ...
  for (const col of ["F", "H", "J"]) {
    ...
    ws.getCell(`${col}${retRow}`).value = { formula: `SUM(${col}${totalRow}*10%)` };
