...
import { computeUtilization, incomingPressure, crewBreakdown } from "@/lib/rules/utilization";
...
export async function getCapacity() {
  ...
  const windows = [
    { key: "4wk", label: "Last 4 weeks", weeks: 4 },
    { key: "3mo", label: "Last 3 months", weeks: 13 },
    { key: "6mo", label: "Last 6 months", weeks: 26 },
  ];
  const utilByWindow = {};
  for (const w of windows) {
    utilByWindow[w.key] = {
      ...computeUtilization({ crew, timecards, realizedHoursPerDay, windowWeeks: w.weeks }),
      label: w.label,
    };
  }

  // Incoming pressure over the next quarter: backlog jobs + weighted pipeline,
  // spread over rough bid-duration. Hours needed = tons*2000 / productivity.
  const HORIZON_WEEKS = 13;
  const backlog = data.projects.filter((p) => p.phase === PHASE.BACKLOG);
  const backlogInputs = backlog.map((p) => {
    const lbs = typeof p.awardedLbs === "number" ? p.awardedLbs : 0;
    const prod = p.bid?.productivity || null;
    const hoursNeeded = prod && prod > 0 ? lbs / prod : 0;
    // rough duration from bid productivity + a nominal crew of 6
    const durWeeks = hoursNeeded > 0 ? Math.max(hoursNeeded / (6 * (realizedHoursPerDay || 6.5) * 5), 2) : HORIZON_WEEKS;
    return { hoursNeeded, durationWeeks: durWeeks };
  });
  // In-flight pipeline: confidence-weighted, and — critically — SPREAD over each
  // job's rough duration and clipped to the horizon, exactly like backlog. The
  // old code dumped every in-flight bid's FULL hours into one quarter with no
  // time-spread, which piled years of maybe-work onto 13 weeks and produced an
  // impossible "~203 crew needed". A bid also only presses if it's likely to
  // START within the horizon, so we weight by confidence (a Submitted bid counts
  // more than an Estimating one) — that weighting already discounts the long shots.
  const inflightInputs = data.bids
    .filter(isInFlight)
    .map((b) => {
      const lbs = typeof b.tons === "number" ? b.tons * 2000 : 0;
      const prod = b.productivity || null;
      const rawHours = prod && prod > 0 ? lbs / prod : 0;
      const hoursNeeded = rawHours * confidenceOf(b.status); // weight by win-likelihood
      const durWeeks = rawHours > 0 ? Math.max(rawHours / (6 * (realizedHoursPerDay || 6.5) * 5), 2) : HORIZON_WEEKS;
      return { hoursNeeded, durationWeeks: durWeeks };
    })
    .filter((x) => x.hoursNeeded > 0);
  const bd = crewBreakdown(crew);
  const pressure = incomingPressure({
    backlogInputs: [...backlogInputs, ...inflightInputs],
    weightedPipelineHours: 0, // now handled per-item with duration spread above
    realizedHoursPerDay,
    windowWeeks: HORIZON_WEEKS,
    headcount: bd.total,
  });
  ...
  return {
    ...
    utilByWindow,
    breakdown: bd,
    realizedHoursPerDay: realizedHoursPerDay || 6.5,
    activeJobCount: data.projects.filter((p) => p.phase === PHASE.RUNNING).length,
    backlogCount: backlog.length,
    incoming: { ...pressure, horizonWeeks: HORIZON_WEEKS },
  };
}
