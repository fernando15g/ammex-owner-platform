...
export async function getEverything() {
  ...
  const { billedWeightToDate, estimatedWeight } = await import("@/lib/rules/lineItems");
  ...
  // Billing, grouped by project — the base of the installed-pounds resolver.
  const eventsByProject = groupEventsByProject(billingEvents);
  const linesByProject = new Map();
  for (const li of allLineItems) {
    if (!li.projectId || li.status === "Closed") continue; // active sheet only — same set billing uses
    if (!linesByProject.has(li.projectId)) linesByProject.set(li.projectId, []);
    linesByProject.get(li.projectId).push(li);
  }
  ...
  for (const p of projects) {
    ...
    // Contract pounds: the live bill sheet is the truth once it exists (Σ LBS-
    // line quantities — the same sheet the billing page bills against). Only jobs
    // with no sheet fall back to the bid's estimated lbs. So adding a line item
    // flows straight into remaining-to-bill and placement %, everywhere.
    const pLines = linesByProject.get(p.id) || [];
