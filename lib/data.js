...
export async function getHome() {
  ...
  for (const r of active.rows) {
    ...
    if (!hit) hit = Object.keys(CITY_COUNTY).find((city) => cc.includes(city)) ? CITY_COUNTY[Object.keys(CITY_COUNTY).find((city) => cc.includes(city))] : null;
  ...
  return {
    ...
    alerts,
    analytics: { bookStages, workMix, foremen, county, unmapped },
  };
}
