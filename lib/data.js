...
export async function getHome() {
  ...
  for (const r of active.rows) {
    ...
    // "needs a location" = can't place as a pin OR a county, and there's no
    // address to geocode either — the jobs to go add site info to.
    if (!pinned && !hit && !isGeocodable(s)) needLocationJobs.push({ id: r.id, name: r.name, projectId: r.projectId });
  }
