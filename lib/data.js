...
function mapProject(page) {
  ...
  return {
    ...
    combineBaseline: getNumber(page, "Combine Baseline"),
...
export async function getEverything() {
  ...
  for (const p of projects) {
    ...
    p.hours = actualHoursForProject(entry, p.payrollHours, { mode: p.hoursMode, baseline: p.combineBaseline, manualOverride: p.manualHoursOverride });
...
export async function getHome() {
  ...
  // ONE source of truth: the project's own coordinates (cached, freshly geocoded,
  // or hand-placed). A pin drives everything — the dot, and the county it shades
  // (point-in-polygon). No pin and no address to geocode = it needs a location.
  for (const r of active.rows) {
    const s = r.site || {};
    let coords = null;
    if (typeof s.lat === "number" && typeof s.lng === "number") {
      coords = { lat: s.lat, lng: s.lng };
    } else if (!s.manual && isGeocodable(s) && geocodeBudget > 0) {
      geocodeBudget -= 1;
      const g = await geocodeAddress(s);
      if (g) { coords = g; try { await updateProject(r.id, { siteLat: g.lat, siteLng: g.lng }); } catch {} }
    }
