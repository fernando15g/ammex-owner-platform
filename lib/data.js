...
export async function getCapacity() {
  ...
  const inflightHours = data.bids
    .filter(isInFlight)
    .reduce((acc, b) => {
      const lbs = typeof b.tons === "number" ? b.tons * 2000 * confidenceOf(b.status) : 0;
      const prod = b.productivity || null;
      return acc + (prod && prod > 0 ? lbs / prod : 0);
    }, 0);
  const bd = crewBreakdown(crew);
  const pressure = incomingPressure({
    backlogInputs,
    weightedPipelineHours: inflightHours,
    realizedHoursPerDay,
