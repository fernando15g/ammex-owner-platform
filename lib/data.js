...
export async function getBook() {
  ...
  const open = awarded.filter((p) => p.phase !== PHASE.COMPLETE).map(toRow);
  ...
  return { open, closed, totals: totalsOf(open), closedTotals: totalsOf(closed) };
}
