...
import { queryAll, bustNotionReadCache, bustNotionReadCacheFor, freshBustIfChanged, probeDatabase, getTitle, getText, getNumber, getDate, getStatus, getSelect, getCheckbox, getMultiSelect, getRelationIds, getRollupNumber, lastEdited, pageId } from "@/lib/notion/client";
import { DB, DB_LABELS } from "@/lib/notion/ids";

// Addresses that failed to geocode, so a bad address doesn't get retried on
// every dashboard visit. Per warm instance; retries naturally after 6h or a
// cold start — this only exists to stop the every-load retry cost.
const _geoFails = new Map();
const GEO_FAIL_TTL = 6 * 60 * 60 * 1000;
const _geoKey = (s) => [s.street, s.city, s.state, s.zip].map((x) => String(x || "").trim().toLowerCase()).join("|");
const _geocodeRecentlyFailed = (s) => {
  const t = _geoFails.get(_geoKey(s));
  return t != null && Date.now() - t < GEO_FAIL_TTL;
};
const _rememberGeocodeFailure = (s) => { _geoFails.set(_geoKey(s), Date.now()); };
...
import { computeUtilization, committedRemaining, crewBreakdown } from "@/lib/rules/utilization";
import { computeWinRate } from "@/lib/rules/winRate";
import { submittedStats } from "@/lib/rules/submissions";
...
function mapProject(page) {
  ...
  return {
    ...
    siteCrossroads: getText(page, "Site Crossroads"),
    supplierPoNotified: getCheckbox(page, "Supplier PO Notified"),
    supplierPoSentDate: getDate(page, "Supplier PO Sent Date"),
    supplierPoSentTo: getText(page, "Supplier PO Sent To"),
    ...
    sitePinManual: getCheckbox(page, "Site Pin Manual"),
    notes: getText(page, "Notes"),
    billingJobReference: getText(page, "Billing Job Reference"), // the GC/fabricator's own job name or number — what goes on the invoice
    hoursMode: getSelect(page, "Hours Mode"),
    ...
    // Read back so the settings form can SEED from it. Without this the form
    // starts blank every time and sends null on save — the only reason that
    // never wiped the stored reason is a guard in billingRepository.
    contractOverrideReason: getText(page, "Contract Override Reason"),
...
export async function getEverything() {
  ...
  const { billedWeightToDate, estimatedWeight, projectLineItems } = await import("@/lib/rules/lineItems");
  const { computeBilling } = await import("@/lib/rules/billing");

  const [bidPages, projectPages, crewPages, timecardPages, recPages, billingEvents, allLineItems] = await Promise.all([
  ...
  // Billing events, grouped by project. Line items are joined per-project below
  // via projectLineItems (matches a line by projectId OR the project's bidIds —
  // the SAME join the billing sheet uses, so bid-attached lines aren't missed).
  const eventsByProject = groupEventsByProject(billingEvents);
  ...
  for (const p of projects) {
    ...
    p.hours = actualHoursForProject(entry, p.payrollHours, { mode: p.hoursMode, manualOverride: p.manualHoursOverride });

    // --- INSTALLED POUNDS RESOLVER (the one source every zone reads) --------
    // Billing dictates installed the moment a job has OS billing: billed LBS
    // weight IS what's installed (you don't bill steel that isn't down).
    // Jobs with no OS billing — the legacy ones — stay on the hand-entered
    // Rebar Placed To-Date, fully editable, exactly as before.
    const pLines = projectLineItems(p, allLineItems);   // projectId OR bidId — same as the sheet
    const pEvents = eventsByProject.get(p.id) || [];
    p.billing = computeBilling(p, pEvents, pLines);      // one billing pass every zone can read
    const sheetWeight = estimatedWeight(pLines);
    const contractLbs = sheetWeight > 0 ? sheetWeight : (p.bid?.estimatedLbs ?? p.estimatedLbsRollup ?? null);
    p.awardedLbs = typeof contractLbs === "number" ? contractLbs : null;

    // Revised contract VALUE ($): the live sheet + change orders when a sheet
    // exists, else the bid — the SAME figure Billing and The Book show, so a
    // job's contract value reads identically on every page.
    p.contractValue = p.billing?.revisedContract > 0 ? p.billing.revisedContract : (p.bid?.contractValue ?? null);

    p.billedLbs = billedWeightToDate(pLines) || 0;
    const dated = pEvents.filter((e) => e.type === "Bill" && (e.amount || 0) > 0 && e.date);
    p.lastBilledDate = dated.length ? dated.map((e) => e.date).sort().slice(-1)[0] : null;
    p.billedPctLbs =
      p.billedLbs > 0 && typeof p.awardedLbs === "number" && p.awardedLbs > 0
        ? p.billedLbs / p.awardedLbs
        : null;

    p.installedSource = p.billedLbs > 0 ? "billed" : "placed";
    p.installedLbs = p.installedSource === "billed" ? p.billedLbs : (typeof p.placedLbs === "number" ? p.placedLbs : null);

    p.financials = makeFinancials({
      contractLbs,
      contractRate: p.bid?.bidRate ?? null,
      contractValue: p.bid?.contractValue ?? null,
      installedLbs: p.installedLbs,
      billableLbs: null,         // approved-to-invoice — still not tracked
      billedLbs: p.billedLbs > 0 ? p.billedLbs : null,
      paidAmount: null,
    ...
    p.placedFraction =
      typeof p.installedLbs === "number" && typeof p.awardedLbs === "number" && p.awardedLbs > 0
        ? p.installedLbs / p.awardedLbs
        : null;
    p.remainingTons =
      typeof p.awardedLbs === "number"
        ? Math.max(p.awardedLbs - (p.installedLbs || 0), 0) / 2000
        : null;
  }
...
export async function getActiveWork() {
  // Projects can change right before landing here — most notably a delete that
  // navigates straight to this page. Refetch projects if they changed since last
  // look, so a just-deleted job doesn't linger from cache. Cheap probe; only
  // busts when something actually changed.
  await freshBustIfChanged(DB.PROJECTS);
  const { inferredStatus } = await import("@/lib/rules/phase");
  const { paceForProject } = await import("@/lib/rules/performance");
  const { getAllBillingEvents, groupEventsByProject } = await import("@/lib/notion/billingRepository");
  const { getAllLineItems } = await import("@/lib/notion/lineItemRepository");
  ...
  const backlog = data.projects
    ...
    .map((p) => ({
      ...
      status: p.status,
      gc: p.bid?.gc || [],
      specialtyTypes: p.bid?.specialtyTypes || [],
      contractValue: p.bid?.contractValue ?? null,
  ...
  const rows = running.map((p) => {
    ...
    return {
      ...
      installedLbs: p.installedLbs,
      installedSource: p.installedSource,       // "billed" | "placed"
      billedLbs: p.billedLbs > 0 ? p.billedLbs : null,
      billedPctLbs: p.billedPctLbs ?? null,
      pace: paceForProject(p),                  // same resolver Performance uses
      ...
      site: { street: p.siteStreet, crossroads: p.siteCrossroads, city: p.siteCity, state: p.siteState, zip: p.siteZip, lat: p.siteLat, lng: p.siteLng, manual: p.sitePinManual },
      notes: p.notes || "",
      supplierPoNotified: !!p.supplierPoNotified,
      supplierPoSentDate: p.supplierPoSentDate ?? null,
      supplierPoSentTo: p.supplierPoSentTo ?? null,
      contractValue: p.contractValue ?? p.bid?.contractValue ?? null,
      ...
      detail: {
        ...
        hoursTimesheet: p.hours?.timesheetHours ?? null,
        hoursPayroll: p.hours?.payrollHours ?? null,
        gc: p.bid?.gc ?? [],
        specialtyTypes: p.bid?.specialtyTypes ?? [],
  ...
  // CLOSED — finished jobs (Closed / Paid → COMPLETE phase). They leave the
  // active list, but shouldn't vanish: shown as a collapsed section at the bottom
  // so you can still find and reopen a job's data after it's done.
  const closed = data.projects
    .filter((p) => p.phase === PHASE.COMPLETE)
    .map((p) => ({
      id: p.id,
      projectId: p.projectId,
      name: p.name,
      ...
      contractValue: p.contractValue ?? p.bid?.contractValue ?? null,
      awardedLbs: p.awardedLbs ?? p.bid?.estimatedLbs ?? null,
      placedLbs: p.installedLbs ?? p.placedLbs ?? null,
      bidId: p.relatedBidId ?? null,
    }))
    .sort((a, b) => String(b.projectId || "~").localeCompare(String(a.projectId || "~"), undefined, { numeric: true }));

  // WAITING ON BILLING — work finished, money not. These are PHASE.BILLING, which
  // this page did not show at all: flipping a job to "Waiting on billing" made it
  // disappear from Active without arriving in Closed. Same vanishing act the
  // backlog section above was added to fix.
  const billing = data.projects
    .filter((p) => p.phase === PHASE.BILLING)
    .map((p) => ({
      id: p.id,
      projectId: p.projectId,
      name: p.name,
  ...
  // CATCH-ALL — anything whose phase is not one of the four groups above. A new
  // or mistyped Project Status option maps to PHASE.UNKNOWN, and an unmapped
  // phase means the job appears NOWHERE. This section guarantees that cannot
  // happen again: if it does not fit a group, it still shows up here.
  const grouped = new Set([PHASE.BACKLOG, PHASE.RUNNING, PHASE.BILLING, PHASE.COMPLETE]);
  const unfiled = data.projects
    .filter((p) => !grouped.has(p.phase))
    .map((p) => ({
      id: p.id,
      projectId: p.projectId,
      name: p.name,
...
export async function getPipeline() {
  // Only the BIDS table reads live here — the estimator app writes bids straight
  // to Notion behind the OS's back, so a stale bid list would miss them. Projects,
  // billing, timecards, and line items are only written THROUGH the OS (which
  // busts the cache on write), so they stay warm-cached and keep this page fast.
  // This was the fix for the bids page going slow: it used to bust ALL seven
  // tables and reload everything live on every visit.
  // Layer 2: only refetch bids if a bid actually changed since last visit. One
  // tiny probe (newest edited row) decides — so a normal navigation serves bids
  // from cache (instant, like the other pages), and we pay for the full bids
  // fetch only when the estimator or the OS actually wrote a bid.
  await freshBustIfChanged(DB.BID_TRACKER);
  const data = await getEverything();
  ...
  const rows = data.bids.map((b) => ({
    ...
    projectType: b.projectType || [],   // searchable: "all the bridge decks I've bid"
    specialtyTypes: b.specialtyTypes || [],   // PT / Mesh tag on the list
    bidDueDate: b.bidDueDate,
    detailer: b.detailer || null,
    submissionDate: b.submissionDate,
    lastFollowUp: b.lastFollowUp,
    contractValue: b.contractValue,
    bidRate: b.bidRate ?? null,   // $/lb — shown on the list as ¢/lb
...
export async function getBillingOverview() {
  ...
  // Every project, for the "+ Invoice" picker — so you can start an invoice from
  // here without hunting for the project first. hasSheet decides the route: with
  // a bid sheet → straight to the invoice; without → to create the sheet first.
  ...
  return { rows, totals, health, allProjects };
}
...
// ---------------------------------------------------------------------------
// ---------------------------------------------------------------------------
// getTimesheetPulse() — this week's field activity for the Home dashboard.
// Reads raw timecards (only place with per-day/per-worker detail), buckets to
// the current Mon–Sun week (+ last week for the trend), and counts only
// non-voided, non-under-review rows — same honesty rules as the burn engine.
// ---------------------------------------------------------------------------
export async function getTimesheetPulse(now = new Date()) {
  const { mapTimecard, isCountable } = await import("@/lib/rules/hours");
  const cards = (await queryAll(DB.TIMESHEET)).map(mapTimecard);
  ...
  let totalHours = 0, lastWeekHours = 0, underReviewHours = 0, unassignedHours = 0;
  const workers = new Set(), jobs = new Set();
  const byDay = [0, 0, 0, 0, 0, 0, 0];

  for (const c of cards) {
    const d = parse(c.date); if (!d) continue;
    const t = d.getTime();
    if (t >= ms(monday) && t < ms(nextMon)) {
      if (c.underReview) underReviewHours += c.hours;
      if (isCountable(c)) {
        totalHours += c.hours;
        if (c.worker) workers.add(c.worker.trim().toLowerCase());
        if (c.projectId) jobs.add(c.projectId); else unassignedHours += c.hours;
        byDay[(d.getDay() + 6) % 7] += c.hours;
      }
    } else if (t >= ms(lastMon) && t < ms(monday) && isCountable(c)) {
      lastWeekHours += c.hours;
    }
  }
  const r = (n) => Math.round(n);

  // Weekly total hours for the last 13 weeks (for the week-over-week variance
  // line under the bar chart). Index 0 = oldest, last = current week.
  const WEEKS = 13;
  const weekly = new Array(WEEKS).fill(0);
  const oldestMon = new Date(monday); oldestMon.setDate(monday.getDate() - (WEEKS - 1) * 7);
  for (const c of cards) {
    if (!isCountable(c)) continue;
    const d = parse(c.date); if (!d) continue;
    const t = d.getTime();
    if (t < ms(oldestMon) || t >= ms(nextMon)) continue;
    const idx = Math.floor((t - ms(oldestMon)) / (7 * 24 * 3600 * 1000));
    if (idx >= 0 && idx < WEEKS) weekly[idx] += c.hours;
  }

  return {
    totalHours: r(totalHours), lastWeekHours: r(lastWeekHours),
    delta: lastWeekHours > 0 ? totalHours / lastWeekHours - 1 : null,
    crew: workers.size, jobs: jobs.size, byDay: byDay.map(r),
    underReviewHours: r(underReviewHours), unassignedHours: r(unassignedHours),
    weekly: weekly.map(r),
  };
}

// getCapacity() — the honest, timesheet-driven crew utilization for the
// dashboard. Multiple windows so you can see "now" (4wk) vs. a quarter vs. half.
export async function getCapacity() {
  const data = await getEverything();
  const crew = data.crew;
  const timecards = data.timecards;
  const realizedHoursPerDay = data.tsSummary?.realizedHoursPerDay ?? null;

  // The one honest read: how busy are we in the last 30 days.
  const util = computeUtilization({ crew, timecards, realizedHoursPerDay });

  // Committed (won) remaining work — active jobs' unplaced lbs + backlog — as a
  // small "what's already on the books" note. NOT speculative pipeline.
  const running = data.projects.filter((p) => p.phase === PHASE.RUNNING);
  const backlog = data.projects.filter((p) => p.phase === PHASE.BACKLOG);
  const committed = committedRemaining({ runningProjects: running, backlogProjects: backlog, realizedHoursPerDay });

  // How many weeks the RUNNING committed work lasts at the current measured pace.
  const weeklyBurn = util.windowDays > 0 ? util.loggedHours / (util.windowDays / 7) : 0;
  const runwayWeeks = weeklyBurn > 0 ? committed.runningRemainingHours / weeklyBurn : null;

  // Backlog CREW pressure — the honest way. What matters for overbooking is NOT
  // a backlog job's total tonnage (it runs for a year+ at a small crew), it's how
  // many crew each needs WHILE running, and how many run concurrently. So we size
  // each backlog job's typical crew from its own scale, then show the pressure as
  // a range: a near-term slice vs. all of them running at once. We can size it;
  // we cannot time it (no start dates) — so it's honestly a range, not a date.
  const backlogHours = committed.remainingHours - committed.runningRemainingHours;
  const hrsPerDay = util.realizedHoursPerDay || 6.9;

  // Per-job typical crew: bigger jobs (by lbs) carry bigger crews. Mid jobs
  // ~5-6, large (1M+ lbs) ~8-10. Derived from lbs, clamped to your real 5-10.
  const backlogJobCrews = backlog.map((p) => {
    const lbs = typeof p.awardedLbs === "number" ? p.awardedLbs : 0;
    if (lbs <= 0) return 5;
    const c = lbs >= 1000000 ? 9 : lbs >= 500000 ? 7 : lbs >= 250000 ? 6 : 5;
    return c;
  });
  const backlogCrewAll = backlogJobCrews.reduce((a, b) => a + b, 0);   // if ALL ran at once
  // near-term: assume roughly a third of backlog jobs plausibly start in-window
  // (you don't stagger deliberately, but they don't all break ground at once)
  const nearTermJobs = Math.max(1, Math.round(backlog.length / 3));
  const backlogCrewNearTerm = backlogJobCrews
    .slice()
    .sort((a, b) => b - a)          // assume the bigger ones move first
    .slice(0, nearTermJobs)
    .reduce((a, b) => a + b, 0);

  return {
    util,
    committed: {
      runningRemainingHours: committed.runningRemainingHours,
      backlogHours,
      runwayWeeks,
      backlogCrewAll,               // all backlog running simultaneously (worst case)
      backlogCrewNearTerm,          // a plausible near-term slice
      nearTermJobs,
    },
...
export async function getHome() {
  const [pipeline, active, billing, book, perf, timesheet, capacity] = await Promise.all([
    getPipeline(), getActiveWork(), getBillingOverview(), getBook(), getPerformance(), getTimesheetPulse(), getCapacity(),
  ]);

  // Win rate needs ALL bids (Awarded/Lost included), not just in-flight. Compute
  // for both a 12-month window and all-time so the dashboard toggle is instant.
  const _everything = await getEverything();
  const winRate = {
    window6: computeWinRate(_everything.bids, { windowMonths: 6 }),
    window12: computeWinRate(_everything.bids, { windowMonths: 12 }),
    allTime: computeWinRate(_everything.bids, { windowMonths: null }),
  };
  ...
  // Cold bids: out for a decision, 14+ days of silence. The clock anchors on the
  // MOST RECENT touch we know of: last follow-up, else submission date, else the
  // BID DUE DATE — because most bids never get a submission date typed in, and
  // Fern almost always submits on the due date. Without that fallback, a bid
  // with no dates could never go cold and sat invisible for months (the Apr/May
  // bids that never flagged). A future due date gives negative daysSince, so
  // not-yet-due bids can't trip the alert. Snoozing sets Last Follow-Up = today,
  // resetting the clock. Sorted hardest-first so the oldest silence tops the list.
  const COLD_DAYS = 14;
  ...
  const placementItems = active.rows
    .filter((r) => (r.installedLbs == null || r.installedLbs === 0))
  ...
  // Closed jobs whose material supplier hasn't been told to close their PO yet
  // (checkbox unticked). Clears as soon as the close-out email is sent.
  const closeoutItems = (_everything.projects || [])
    .filter((p) => p.phase === PHASE.COMPLETE && !p.supplierPoNotified)
    .map((p) => ({ id: p.id, projectId: p.projectId, name: p.name, site: p.site }));
  ...
  if (closeoutItems.length) alerts.push({ id: "closeout", sev: "info", label: `${closeoutItems.length} closed job${closeoutItems.length === 1 ? "" : "s"} — notify supplier to close PO`, count: closeoutItems.length, items: closeoutItems });
  ...
  for (const r of perf.trusted) {
    if (typeof r.placedLbs !== "number" || typeof r.hours !== "number" || r.hours <= 0) continue; // perf rows: placedLbs IS the resolved installed (ws.lbs)
    for (const f of r.foreman || []) {
      const c = fm.get(f) || { name: f, lbs: 0, hours: 0, bidSum: 0, bidN: 0, jobs: 0 };
      c.lbs += r.placedLbs; c.hours += r.rebarHours ?? r.hours; c.jobs += 1; // rebar hours: PT/mesh time must not dilute a foreman's lbs/MH
  ...
  // What the scorecard is NOT counting, and why — a silent filter looks like a
  // bug ("I know I have more foremen than this"). Completed jobs with a foreman
  // that failed the data-trust gate are tallied with their reasons, so the card
  // can say "6 excluded (no hours, stale placed lbs)" and point at what to fix.
  const foremanExcluded = [];
  for (const r of perf.needsReview || []) {
    // needs-review is only assigned to FINISHED jobs (in-progress rows carry
    // their own state), so no done-check is needed here.
    if (!(r.foreman || []).length) continue;     // no foreman -> was never a candidate
    foremanExcluded.push({ name: r.name, foreman: r.foreman, problems: r.problems || [] });
  }
  const foremanExclusionSummary = (() => {
    if (!foremanExcluded.length) return null;
    const reasons = new Map();
    for (const x of foremanExcluded) for (const p of x.problems) {
      const key = p.includes("hours look too low") ? "hours look too low for the tonnage"
        : p.includes("hours look too high") ? "hours look too high for the tonnage"
        : p.includes("no labor hours") ? "no labor hours recorded"
        : p.includes("sample too small") ? "sample too small"
        : p.includes("no placed weight") ? "no placed weight" : p;
      reasons.set(key, (reasons.get(key) || 0) + 1);
    }
    const top = [...reasons.entries()].sort((a, b) => b[1] - a[1]).slice(0, 2).map(([k]) => k);
    return { count: foremanExcluded.length, reasons: top, jobs: foremanExcluded };
  })();
  ...
  const { updateProject } = await import("@/lib/notion/projectRepository");
  const { countyOfPoint } = await import("@/app/home/azCounties");
  const county = {};
  const pins = [];
  const needLocationJobs = [];

  // GEOCODING NO LONGER BLOCKS THE DASHBOARD. It used to run up to 8 addresses
  // sequentially — Census then Nominatim, 6s timeout each, plus a Notion write —
  // inside this render, and an address that FAILED retried on every load. A few
  // bad addresses made the home page take 8-10 seconds forever. Now the render
  // uses only coordinates that already exist; unresolved addresses are geocoded
  // in the BACKGROUND (fire-and-forget) and written to the project, so their
  // pins appear on the next visit. Failures are remembered for a while so a bad
  // address can't keep costing time.
  const toResolve = [];
  for (const r of active.rows) {
    const s = r.site || {};
    let coords = null;
    ...
    } else if (!s.manual && isGeocodable(s) && !_geocodeRecentlyFailed(s)) {
      toResolve.push(r); // resolve after render, not during it
    }

    if (coords) {
      pins.push({ name: r.name, lat: coords.lat, lng: coords.lng });
      const cty = countyOfPoint(coords.lng, coords.lat);
      if (cty) county[cty] = (county[cty] || 0) + 1;
    } else if (!isGeocodable(s)) {
      needLocationJobs.push({
        id: r.id, name: r.name, projectId: r.projectId,
        site: { street: s.street || "", city: s.city || "", state: s.state || "AZ", zip: s.zip || "" },
      });
    }
  }

  // Fire-and-forget: capped, sequential (kind to the free geocoders), never awaited.
  if (toResolve.length) {
    void (async () => {
      for (const r of toResolve.slice(0, 8)) {
        try {
          const g = await geocodeAddress(r.site || {});
          if (g) await updateProject(r.id, { siteLat: g.lat, siteLng: g.lng });
          else _rememberGeocodeFailure(r.site || {});
        } catch { _rememberGeocodeFailure(r.site || {}); }
      }
    })();
  }
  ...
  return {
    tiles: {
      ...
      // bids put out the door this week / month, regardless of later outcome
      submitted: submittedStats(_everything.bids),
    },
    alerts,
    analytics: { bookStages, workMix, foremen, foremanExclusionSummary, county, pins, needLocationJobs, timesheet },
    capacity,
    winRate,
  };
}
...
export async function getBook() {
  ...
  const { computeBilling } = await import("@/lib/rules/billing");
  const { projectLineItems } = await import("@/lib/rules/lineItems");
  const { realizedProfitForProject } = await import("@/lib/rules/performance");
  const [data, events, allLines] = await Promise.all([getEverything(), getAllBillingEvents(), getAllLineItems()]);
  const byProject = groupEventsByProject(events);
  const activeLines = allLines.filter((li) => li.status !== "Closed");
  ...
  const toRow = (p) => {
    ...
    const bidProfit = p.bid?.operatingProfit ?? null;
    const bidMargin = p.bid?.operatingMargin ?? null;
    // Closed jobs show REALIZED profit (what was actually earned) so "profit
    // earned" is truthful and matches the Performance modal. If realized can't
    // be computed (missing placed weight/hours), fall back to the bid figure.
    const isClosed = p.phase === PHASE.COMPLETE;
    const realized = isClosed ? realizedProfitForProject(p) : null;
    const operatingProfit = realized ? realized.realizedProfit : bidProfit;
    const operatingMargin = realized ? realized.realizedMargin : bidMargin;
    return {
      id: p.id,
      projectId: p.projectId,
      name: p.name,
      ...
      gc: p.bid?.gc || [],
      specialtyTypes: p.bid?.specialtyTypes || [],
      relatedBidId: p.relatedBidId || null, // Bid link in the chooser was always grayed out because this was never passed through
      contract,
      operatingProfit,       // closed = realized; active/backlog = bid-expected
      operatingMargin,
      isRealized: !!realized, // client labels "earned" vs "estimated/expected"
      bidProfit,             // the original bid figure, for reference/fallback
      billed,
...
export async function getProjectBilling(projectId) {
  ...
  return {
    ...
    // The bid, start date and RESOLVED hours ride along so read-only views built
    // on this (the project-details modal) can show bid rate, labour hours and
    // start date instead of blanks. Hours come from the same resolution Active
    // Work uses, so the two never disagree.
    bid: project.bid || null,
    actualStartDate: project.actualStartDate ?? null,
    hours: project.hours ?? null,
    payrollHours: project.payrollHours ?? null,
    ...
    settings: {
      ...
      contractOverrideReason: project.contractOverrideReason || "",
    },
...
export async function getProjectAdmin(projectId = null) {
  ...
  const econOf = (b) => {
    ...
    try {
      const inputs = {
        weightLb: b.estimatedLbs,
        ptSpecialty: b.ptSpecialty ?? 0,
        specialtyRevenue: b.specialtyRevenue ?? 0,
        specialtyCost: b.specialtyCost ?? 0,
        specialtyHours: b.specialtyHours ?? 0,
      };
...
export async function getPerformance() {
  ...
  // The installed/billed resolution now happens centrally in getEverything —
  // every project already carries billedLbs / billedPctLbs / installedSource /
  // lastBilledDate, so the engine needs no separate billing context.
  const data = await getEverything();
  return computePerformance(data.projects);
}
