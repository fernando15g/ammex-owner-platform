...
import { queryAll, bustNotionReadCache, probeDatabase, getTitle, getText, getNumber, getDate, getStatus, getSelect, getCheckbox, getMultiSelect, getRelationIds, getRollupNumber, lastEdited, pageId } from "@/lib/notion/client";
...
export async function getPipeline() {
  // The bids page always reads LIVE. Bids can be written by the estimator
  // calculator (a separate app) straight to Notion, and by editing dates here —
  // the cache can't see the calculator's writes, and a stale bids list that
  // doesn't reflect a just-changed Submitted date is confusing. Everywhere else
  // stays cached; only this high-churn, cross-app view bypasses it.
  bustNotionReadCache();
  const data = await getEverything();
