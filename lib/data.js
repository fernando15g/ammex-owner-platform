...
export async function getBillingOverview() {
  ...
  // Bill any project that has a contract value OR any billing events.
  const rows = data.projects
    .filter((p) => (p.billingContractValue || 0) > 0 || byProject.has(p.id))
    .map((p) => {
  ...
  // sort: overdue first, then most outstanding
  rows.sort((a, b) => (b.billing.overdueTotal - a.billing.overdueTotal) || (b.billing.outstanding - a.billing.outstanding));
