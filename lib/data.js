...
export async function getHome() {
  ...
  const coldBids = pipeline.rows
    ...
    .map((r) => {
      ...
      return { id: r.id, name: r.name, gc: r.gc, contractValue: r.contractValue, coldDays: daysSince(anchor) };
    })
    .filter((r) => r.coldDays != null && r.coldDays >= COLD_DAYS)
    .sort((a, b) => b.coldDays - a.coldDays);

  const overPace = active.counts.atRisk || 0;
  const aging = billing.totals.aging || {};
  const overdue60 = (aging.d61_90 || 0) + (aging.d90_plus || 0);
  const missingPlacement = active.rows.filter((r) => r.placedLbs == null || r.placedLbs === 0).length;

  const alerts = [];
  if (overdue60 > 0) alerts.push({ id: "overdue", sev: "danger", text: `${m(overdue60)} past 60 days`, href: "/billing" });
  if (overPace > 0) alerts.push({ id: "overpace", sev: "danger", text: `${overPace} job${overPace === 1 ? "" : "s"} over pace`, href: "/active" });
  if (coldBids.length > 0) alerts.push({ id: "cold", sev: "warn", text: `${coldBids.length} bid${coldBids.length === 1 ? "" : "s"} gone cold`, href: "/pipeline", bids: coldBids });
  if (missingPlacement > 0) alerts.push({ id: "placement", sev: "warn", text: `${missingPlacement} job${missingPlacement === 1 ? "" : "s"} missing placement`, href: "/active" });

  return {
    tiles: {
      pipeline: { weighted: pipeline.totals.weighted, count: pipeline.totals.count },
      active: { running: active.counts.total, overPace },
      billing: { outstanding: billing.totals.outstanding, overdue60 },
