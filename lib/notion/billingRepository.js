...
function labelFor(e) {
  const d = e.date ? (() => { const m = String(e.date).match(/^(\d{4})-(\d{2})-(\d{2})/); const dt = m ? new Date(+m[1], +m[2]-1, +m[3]) : new Date(e.date); return dt.toLocaleDateString("en-US", { month: "short", day: "numeric" }); })() : "";
