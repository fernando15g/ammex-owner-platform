...
const P = {
  ...
  siteCrossroads: "Site Crossroads",
  supplierPoNotified: "Supplier PO Notified",
  supplierPoSentDate: "Supplier PO Sent Date",
  supplierPoSentTo: "Supplier PO Sent To",
  siteCity: "Site City",
  siteState: "Site State",
  siteZip: "Site Zip",
  siteLat: "Site Lat",
  siteLng: "Site Lng",
  sitePinManual: "Site Pin Manual",
  notes: "Notes",
  billingJobReference: "Billing Job Reference",
  hoursMode: "Hours Mode",
  payrollHours: "Labor Hours To-Date",
  manualHoursOverride: "Manual Hours Override",
};
...
function toProps(p) {
  ...
  // A project can carry MORE THAN ONE bid — Phase 2 of a job is bid separately
  // but billed on the same contract. Singular is still accepted (it's what most
  // callers send) and is treated as a list of one.
  if ("relatedBidIds" in p) {
    props[P.relatedBid] = { relation: (p.relatedBidIds || []).map((id) => ({ id })) };
  } else if ("relatedBidId" in p) {
    props[P.relatedBid] = { relation: p.relatedBidId ? [{ id: p.relatedBidId }] : [] };
  }
  if ("placedLbs" in p) props[P.placedLbs] = fmt.number(p.placedLbs);
  if ("siteStreet" in p) props[P.siteStreet] = fmt.richText(p.siteStreet);
  if ("siteCrossroads" in p) props[P.siteCrossroads] = fmt.richText(p.siteCrossroads);
  if ("supplierPoNotified" in p) props[P.supplierPoNotified] = { checkbox: !!p.supplierPoNotified };
  if ("supplierPoSentDate" in p) props[P.supplierPoSentDate] = fmt.date(p.supplierPoSentDate);
  if ("supplierPoSentTo" in p) props[P.supplierPoSentTo] = fmt.richText(p.supplierPoSentTo);
  if ("siteCity" in p) props[P.siteCity] = fmt.richText(p.siteCity);
  if ("siteState" in p) props[P.siteState] = fmt.richText(p.siteState);
  if ("siteZip" in p) props[P.siteZip] = fmt.richText(p.siteZip);
  if ("siteLat" in p) props[P.siteLat] = fmt.number(p.siteLat);
  if ("siteLng" in p) props[P.siteLng] = fmt.number(p.siteLng);
  if ("sitePinManual" in p) props[P.sitePinManual] = fmt.checkbox(p.sitePinManual);
  if ("notes" in p) props[P.notes] = fmt.richText(p.notes);
  if ("billingJobReference" in p) props[P.billingJobReference] = fmt.richText(p.billingJobReference);
  if ("hoursMode" in p) props[P.hoursMode] = fmt.select(p.hoursMode);
  if ("payrollHours" in p) props[P.payrollHours] = fmt.number(p.payrollHours);
  if ("manualHoursOverride" in p) props[P.manualHoursOverride] = fmt.checkbox(p.manualHoursOverride);
  return props;
}
...
// Lean check: which projects (if any) are built on a given bid. Queries the
// Projects DB filtered by the "Related Bid" relation instead of reading the
// entire workspace — the old attachment check called getEverything() on every
// mark-lost, which is what made it slow. Returns [{ id, name }].
export async function projectsForBid(bidId) {
  const pages = await queryAll(DB.PROJECTS, {
    property: P.relatedBid,
    relation: { contains: bidId },
  });
  return pages.map((pg) => ({
    id: pg.id,
    name: pg.properties?.[P.name]?.title?.[0]?.plain_text || "(unnamed project)",
  }));
}
