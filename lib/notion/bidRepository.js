...
const P = {
  ...
  hoursPerDay: "Hours Per Day (calc)",
  targetMarginPct: "Target Margin % (calc)",
  // Specialty scope (PT + mesh), written by the bid calculator. Labor-only, and
  // deliberately separate from the rebar figures so neither distorts the other.
  rebarRevenue: "Rebar Revenue (calc)",
  specialtyRevenue: "Specialty Revenue (calc)",
  specialtyCost: "Specialty Cost (calc)",
  specialtyHours: "Specialty Hours (calc)",
  specialtyType: "Specialty Type",   // multi-select: PT Bridge / PT Building / Mesh
  detailer: "Detailer",              // select — who detailed the job; auto-creates options on write
  // Out-of-town (travel) add-on — mirrors the calculator's column names exactly.
  // Inputs the user set:
  travelOn: "Travel On (calc)",
  hotelRooms: "Hotel Rooms (calc)",
  hotelNightlyRate: "Hotel Nightly Rate (calc)",
  hotelNights: "Hotel Nights (calc)",
  hotelTaxPct: "Hotel Tax % (calc)",              // Percent column: stores the ratio
  hotelNightsBasis: "Hotel Nights Basis (calc)",  // 5 or 7
  fuelMiles: "Fuel Miles (calc)",
  fuelTrips: "Fuel Trips (calc)",
  fuelMPG: "Fuel MPG (calc)",
  fuelPerGal: "Fuel Per Gal (calc)",
  subsistenceRate: "Subsistence Rate (calc)",
  subsistenceInLabor: "Subsistence In Labor (calc)",
  travelMarkupOn: "Travel Markup On (calc)",
  travelAddToBid: "Travel Add To Bid (calc)",
  // Overtime
  otPct: "OT %",
  otCentsPerLb: "OT \u00a2/lb (calc)",
  otHours: "OT Hours (calc)",
  otPremium: "OT Premium (calc)",  // is travel folded into the quoted rate?
  travelMarkupPct: "Travel Markup % (calc)",      // Percent column: stores the ratio
  // Computed outputs:
  hotelCost: "Hotel Cost (calc)",
  fuelCost: "Fuel Cost (calc)",
  subsistenceCost: "Subsistence Cost (calc)",
  travelTotal: "Travel Total (calc)",
  travelAddOnCents: "Travel Add-On Cents (calc)",
};
...
function toNotionProps(m) {
  ...
  if ("lastFollowUp" in m) props[P.lastFollowUp] = fmt.date(m.lastFollowUp);
  ...
  // Travel add-on. Key-presence for the checkboxes (false is a real value that
  // must be writable); != null for the numbers, same as every other input here.
  // Nothing is written unless the caller sent it, so a status-only PATCH can
  // never blank these.
  if ("travelOn" in m) props[P.travelOn] = fmt.checkbox(m.travelOn);
  if ("subsistenceInLabor" in m) props[P.subsistenceInLabor] = fmt.checkbox(m.subsistenceInLabor);
  if ("travelMarkupOn" in m) props[P.travelMarkupOn] = fmt.checkbox(m.travelMarkupOn);
  if ("travelAddToBid" in m) props[P.travelAddToBid] = fmt.checkbox(m.travelAddToBid);
  for (const k of ["otPct", "otCentsPerLb", "otHours", "otPremium"]) {
    if (m[k] != null) props[P[k]] = fmt.number(m[k]);
  }
  for (const k of ["hotelRooms","hotelNightlyRate","hotelNights","hotelTaxPct","hotelNightsBasis",
                   "fuelMiles","fuelTrips","fuelMPG","fuelPerGal","subsistenceRate","travelMarkupPct",
                   "hotelCost","fuelCost","subsistenceCost","travelTotal","travelAddOnCents"]) {
    if (m[k] != null) props[P[k]] = fmt.number(m[k]);
  }
  ...
  if (m.hoursPerDay != null) props[P.hoursPerDay] = fmt.number(m.hoursPerDay);
  if (m.targetMarginPct != null) props[P.targetMarginPct] = fmt.number(m.targetMarginPct);
  if (m.rebarRevenue != null) props[P.rebarRevenue] = fmt.number(m.rebarRevenue);
  if (m.specialtyRevenue != null) props[P.specialtyRevenue] = fmt.number(m.specialtyRevenue);
  if (m.specialtyCost != null) props[P.specialtyCost] = fmt.number(m.specialtyCost);
  if (m.specialtyHours != null) props[P.specialtyHours] = fmt.number(m.specialtyHours);
  if ("specialtyTypes" in m) props[P.specialtyType] = fmt.multiSelect(m.specialtyTypes || []);
  if ("detailer" in m) props[P.detailer] = fmt.select(m.detailer || null);
  return props;
}
