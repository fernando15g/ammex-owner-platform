...
const P = {
  ...
  productivity: "Productivity",    // optional per-line output rate (specialty scopes)
  specialtyType: "Specialty Type", // EXPLICIT specialty marker — only the specialty editor sets this
...
export function mapLineItem(page) {
  return {
    ...
    // Optional per-line output rate for specialty scopes (lb/MH, sqft/MH).
    // Blank until the Notion property exists — the type default fills in.
    productivity: getNumber(page, P.productivity),
    specialtyType: getSelect(page, P.specialtyType) || null,
...
function toProps(li) {
  ...
  if ("specialtyType" in li && li.specialtyType) props[P.specialtyType] = fmt.select(li.specialtyType);
