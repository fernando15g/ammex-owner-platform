...
// OT premium for specialty labor: the extra half on the OT share of these hours,
// loaded through burden and compounded through tools + contingency to match how
// specialty cost (costPerMH) is built. Zero when OT is off.
// Ported verbatim from the calculator (lib/calc.js specialtyOTPremium).
export function specialtyOTPremium(hours, i) {
  if (!i.otOn || !hours) return 0;
  const loadedRate = i.wageRate * (1 + i.burdenPct);
  return hours * (i.otPct || 0) * 0.5 * loadedRate * (1 + i.toolsPct) * (1 + i.contingencyPct);
}
...
export function computeSpecialtyLine(line, i) {
  ...
  if (line.type === "PT Building") {
    ...
    if (!blank(line.prodLbPerMH) && lbs > 0) {
      ...
      out.otPremium = specialtyOTPremium(out.hours, i);
      out.cost = out.hours * M + out.otPremium;
    ...
    if (hrs > 0) {
      ...
      out.otPremium = specialtyOTPremium(hrs, i);
      out.cost = hrs * M + out.otPremium;
      out.hasCostBasis = true;
...
export function specialtyTypeOfLine(li) {
  if (!li) return null;
  // EXPLICIT stamp first — only the specialty editor writes Specialty Type, so
  // this can never misfire on a line you typed or pasted on the bid sheet.
  if (li.specialtyType && SPECIALTY_TYPES.includes(li.specialtyType)) return li.specialtyType;
  if (li.billingBasis === "Hours") return "PT Bridge"; // hourly COs, deliberate
  // Legacy fallback for lines created before the stamp existed: only "PT LBS",
  // a unit nothing but the specialty editor ever wrote. SF and HRS are REAL
  // billing units (ADOT bills in square feet; hourly lines exist) — inferring
  // specialty from them is exactly the bug this fix removes.
