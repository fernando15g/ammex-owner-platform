...
export function priceBid(rawInputs, activeRatePerLb = null) {
  ...
  return {
    ...
    totalMH: e.totalMH,
    // OT on the REBAR side. The premium is already inside fullyLoadedCost; these
    // are passed through so the screen can show "Estimated OT" and store the
    // (calc) columns. Without them the UI saw undefined and reported only the
    // specialty share.
    otPremium: e.otPremium,
    otCentsPerLb: e.otCentsPerLb,
    otHours: e.otHours,
    fieldMH: e.fieldMH,
    crewDays: e.crewDays,
    assumptions: {
