...
export function classifyJob(p, ctx) {
  ...
  let matched = null;
  if (ws.source === "billed" && ws.lastBilledDate && p.hours?.era === "timesheet") {
    const hrsThrough = hoursThroughDate(p.timecards, ws.lastBilledDate);
    if (hrsThrough && hrsThrough > 0 && ws.billedLbs > 0) {
      matched = { lbsPerMH: ws.billedLbs / hrsThrough, hours: hrsThrough, throughDate: ws.lastBilledDate };
    }
  }

  // realized: matched when available (same-window honesty), else lbs ÷ all hours
  const allHoursRealized = placed && hours ? placed / hours : null;
  const realized = matched ? matched.lbsPerMH : allHoursRealized;
