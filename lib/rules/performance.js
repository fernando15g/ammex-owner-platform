...
export function paceForProject(p, { done = false } = {}) {
  const totalHours = typeof p.hours?.hours === "number" && p.hours.hours > 0 ? p.hours.hours : null;
  const installed = typeof p.installedLbs === "number" && p.installedLbs > 0 ? p.installedLbs : null;
  if (!installed || !totalHours) return null;

  // lbs/MH measures REBAR placement. Specialty pounds are (correctly) excluded
  // from the numerator, so specialty HOURS must come out of the denominator too
  // — otherwise a job with PT reads slow purely because PT hours are in there,
  // and a heavily-PT job can fall under the sanity floor and get thrown out as
  // bad data. Timesheets don't split PT from rebar yet, so the bid's specialty
  // hours are the best available estimate. If they'd swallow the whole job the
  // figure is untrustworthy, so we leave it alone and let the trust gate speak.
  const specHours = Number(p.bid?.specialtyHours) || 0;
  const hours = specHours > 0 && totalHours > specHours ? totalHours - specHours : totalHours;
  const specialtyAdjusted = hours !== totalHours;

  if (p.installedSource !== "billed") {
    return { lbsPerMH: installed / hours, source: "placed", throughDate: null, specialtyAdjusted };
  }
  ...
  if (!nearComplete && p.lastBilledDate && p.hours?.era === "timesheet") {
    let hrsThrough = hoursThroughDate(p.timecards, p.lastBilledDate);
    // same correction on the through-date slice, pro-rated by how much of the
    // job's hours it covers
    if (hrsThrough && specialtyAdjusted && totalHours > 0) {
      hrsThrough = Math.max(hrsThrough - specHours * (hrsThrough / totalHours), 0);
    }
    if (hrsThrough && hrsThrough > 0) {
      const lbsPerMH = installed / hrsThrough;
      const hoursSince = Math.max(0, hours - hrsThrough);
      return {
        lbsPerMH, source: "matched", throughDate: p.lastBilledDate, specialtyAdjusted,
  ...
  return { lbsPerMH: installed / hours, source: "total", throughDate: null, specialtyAdjusted };
}
...
export function classifyJob(p, ctx) {
  ...
  const specHoursBid = Number(p.bid?.specialtyHours) || 0;
  const rebarHours = specHoursBid > 0 && hours != null && hours > specHoursBid ? hours - specHoursBid : hours;
  const allHoursRealized = placed && rebarHours ? placed / rebarHours : null;
  ...
  const base = {
    ...
    phase: p.phase,
    placedLbs: placed,
    // Hours with specialty removed — the denominator for anything measuring
    // REBAR output. `hours` stays the job total (correct for cost).
    rebarHours,
    ...
    // Specialty (PT/mesh) rides alongside rebar and doesn't scale with placed
    // pounds — realizedEconomics separates the two.
    specialtyRevenue: p.bid?.specialtyRevenue ?? null,
    specialtyCost: p.bid?.specialtyCost ?? null,
    specialtyHours: p.bid?.specialtyHours ?? null,
...
//
// REBAR and SPECIALTY are handled separately, because they don't scale together:
//   • Rebar is paid by placed weight, so an underrun shrinks it proportionally.
//   • Specialty (PT, mesh) was either performed or it wasn't — it doesn't shrink
//     because fewer rebar pounds went in. Baking it into a per-pound rate made
//     PT revenue melt away on an underrun, which is simply wrong.
//
//   revenue = placed × rebar $/lb  +  specialty revenue
//   profit  = the bid's profit for what was done (rebar portion scaled, specialty
//             whole), adjusted for how actual labor compared to budget.
//   margin  = profit ÷ revenue
//
// Hours are TOTAL for the job — timesheets don't separate PT from rebar. So the
// budget side must be total too: rebar hours for the placed pounds PLUS the
// specialty hours the bid carried. Comparing total actual against rebar-only
// budget would make any job with PT look like it blew its hours.
//
// Nothing stored; re-derives if hours or weight are corrected later.
export function realizedEconomics(row) {
  const { contractValue, operatingProfit, operatingMargin, awardedLbs, placedLbs, bidProductivity, hours, baseWage } = row;
  const specRevenue = Number(row.specialtyRevenue) || 0;
  const specCost = Number(row.specialtyCost) || 0;
  const specHours = Number(row.specialtyHours) || 0;
  const specProfit = specRevenue > 0 ? specRevenue - specCost : 0;
  if (
  ...
  const scopeFraction = placedLbs / awardedLbs;           // portion of AWARDED REBAR actually placed

  // Split the bid into its rebar and specialty halves. contractValue and
  // operatingProfit are combined figures; awardedLbs is rebar-only, so the
  // rebar rate has to come off the rebar revenue alone.
  const rebarRevenue = Math.max(contractValue - specRevenue, 0);
  const rebarProfit = operatingProfit - specProfit;
  const rate = rebarRevenue / awardedLbs;                 // true rebar $/lb

  // Revenue: rebar scales with what was placed; specialty is taken whole.
  const revenue = placedLbs * rate + specRevenue;

  // Profit the bid expected for what actually happened.
  const bidProfitAtScope = rebarProfit * scopeFraction + specProfit;

  // Labor: total actual hours vs total budgeted hours (rebar for the placed
  // pounds + the specialty hours the bid carried), so both sides count the same
  // work. Once PT hours are tracked separately this can be split too.
  const bidHoursForPlaced = placedLbs / bidProductivity + specHours;
  ...
  return {
    ...
    specRevenue,
    specHours,
    hasSpecialty: specRevenue > 0 || specHours > 0,
    bidProfitAtScope,
    laborDelta,
    realizedProfit,
    realizedMargin,
...
// realizedProfitForProject(p, ctx) — the Book's closed rows use this to show
// REALIZED profit/margin (what the job actually earned), identical to what the
// Performance modal shows. Returns null if the job lacks the inputs to compute
// it (no placed weight, no hours, etc.), in which case the Book falls back to
// the bid figure. Mirrors the base-row construction in computeProjectRow so the
// two views can never disagree on a finished job.
export function realizedProfitForProject(p, ctx = {}) {
  const ws = weightSourceForProject(p, ctx);
  const placed = ws.lbs;
  const hours = typeof p.hours?.hours === "number" && p.hours.hours > 0 ? p.hours.hours : null;
  const row = {
  ...
  const econ = realizedEconomics(row);
  if (!econ) return null;
  return { realizedProfit: econ.realizedProfit, realizedMargin: econ.realizedMargin };
}
...
export function computePerformance(projects, billingByProject = null) {
  ...
  const totHours = trusted.reduce((a, r) => a + (r.rebarHours ?? r.hours), 0);
