...
export function reconcile({ projects = [], events = [], lines = [] }) {
  ...
  for (const proj of projects) {
    ...
    // Specialty priced on the bid but absent from the sheet. Contract value comes
    // from line items once a sheet exists, so the PT/mesh money would quietly
    // drop out of the job's contract. Flag it rather than guess a correction.
    const bidSpecialty = Number(proj.bid?.specialtyRevenue) || 0;
    if (bidSpecialty > 0 && projLines.length > 0) {
      const hasSpecialtyLine = projLines.some((l) => {
        const u = String(l.unit || "").toUpperCase();
        return l.billingBasis === "Hours" || u === "PT LBS" || u === "SF" || u === "SQFT" || u === "HRS";
      });
      if (!hasSpecialtyLine) {
        add(SEVERITY.WARNING, "specialty-missing-from-sheet",
          `"${proj.name}" was bid with $${Math.round(bidSpecialty).toLocaleString()} of specialty (PT/mesh), but its bid sheet has no specialty lines — that revenue isn't in the contract or billable.`,
          { project: proj.name });
      }
    }
