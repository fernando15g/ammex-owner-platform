...
export function reconcile({ projects = [], events = [], lines = [] }) {
  ...
  for (const proj of projects) {
    ...
    const projLines = lines.filter(
      (l) => l.projectId === proj.id || (proj.relatedBidId && l.bidId === proj.relatedBidId)
    );
    const billed = evts.filter((e) => e.type === "Bill").reduce((a, e) => a + (e.amount || 0), 0);
    ...
    const outstanding = billed - paid - rolled;
