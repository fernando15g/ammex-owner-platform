...
export function actualHoursForProject(summaryEntry, payrollHours, opts = {}) {
  ...
  const { mode, manualOverride } = typeof opts === "boolean" ? { manualOverride: opts } : opts;
  ...
  const resolved = ms.includes("payroll") ? "payroll"
    : ms.includes("combin") ? "payroll"   // Combined retired: any legacy value resolves to Payroll (no double-count)
  ...
  // COMBINED: payroll baseline + the full timesheet total. Payroll stays frozen
  // during the job; at close, flip to Payroll and enter accounting's final number
  // (which overwrites this estimate). "Add both and count both."
  // "combined" is retired — resolveMode maps any legacy value to "payroll", so
  // this double-counting branch is dead. Left removed on purpose.
