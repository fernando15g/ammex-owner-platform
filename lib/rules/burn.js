...
export function computeBurn(project) {
  ...
  // Budgeted hours are REBAR-only (lbs ÷ productivity + mobilization), but actual
  // hours are the whole job. On a job with PT that made burn look blown from day
  // one, so the bid's specialty hours are added to the budget — total vs total.
  const specHours = Number(bid?.specialtyHours) || 0;
  const rebarBudget = bid?.projectedHours ?? null;
  const projectedHours = typeof rebarBudget === "number" ? rebarBudget + specHours : null;
