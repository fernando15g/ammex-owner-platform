...
export function planShortPayUnwind(payment, invoice) {
  ...
  return {
    // put the rolled-back quantities BACK on the lines
    lineRestores: (carry.lines || []).map((l) => ({ id: l.id, addQty: l.qty || 0 })),
...
// -----------------------------------------------------------------------------
const PROJECT_EDITABLE = ["name", "projectId", "status", "actualStartDate", "foreman", "relatedBidId"];
